...
//		jsonObject = rest.postForObject("http://localhost:8083/api/product2", request, JSONObject.class);
	...
	@GetMapping(value  = "api/edit")
	public ModelAndView edit(ModelAndView m	,@Param (value = "id") int id,@Param(value = "name") String name,@Param(value = "soluong") int soluong ) {
		...
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(url)
				.queryParam("id",id)
				.queryParam("name",name).queryParam("soluong", soluong);
		HttpEntity<?> entity = new HttpEntity<>(headers);
		HttpEntity<String> response = rest.exchange(builder.toUriString(), HttpMethod.GET, entity, String.class);
		org.json.JSONObject j = new org.json.JSONObject(response.getBody());
		if(j.get("status").toString().equalsIgnoreCase("okay")) {
			m.addObject("error", "sua thanh cong");
		
		}
		else {
			m.addObject("error", "sua that bai");
			
		}
		m.setViewName("redirect:../product");
